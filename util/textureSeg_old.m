function imgs_filtered = textureSeg_old( img,filter_bank )
...
%     cluster(cluster==0) = [];
    ...
    tex_rm_nonblack_filtered = removeLabelNoise(tex_rm_nonblack);
    tex_rm_nonblack_filtered = morphology(tex_rm_nonblack_filtered,2,'close');
    tex_rm_nonblack_filtered = removeSmallRegions(tex_rm_nonblack_filtered,300);
%     r_thr = 0.35;
%     g_thr = 0.35;
%     b_thr = 0.4;
%     for i=1:numel(cluster)
...
%     cluster(cluster==0) = [];
    ...
    tex_rm_highfreq = tex_rm_nonblack_filtered;
...
%     img_DoG = imadjustRGB(img_DoG);
    ...
    tex_rm_highfreq(~(img_entropy_white_px | img_entropy_black_px | img_entropy_gray_px)) = 0;
...
%     tex_clean = morphology(tex_clean,2,'close');
    ...
    imgs_filtered.tex_rm_nonblack_filtered = tex_rm_nonblack_filtered;
